# Ainulindalë: Lost Tales documentation

This documentation describes the systems that are implemented in the Minecraft 1.7.10 Forge backport. It is a source reference, not a list of planned features.

Start with [the project overview](wiki/Ainulindalë_Lost_Tales.md), then use the pages below for details.

- [Roleplay characters](wiki/RP-Characters.md)
- [Playable races](wiki/Races.md)
- [Factions](wiki/Factions.md)
- [Parties](wiki/Parties.md)
- [Quests and missives](wiki/Quests.md)
- [Map, HUD, controls, and third-person camera](wiki/Map_HUD_and_Controls.md)
- [Accessories and the One Ring](wiki/Accessories_and_the_One_Ring.md)
- [Items, blocks, entities, and world content](wiki/Content_and_World.md)
- [3D models](wiki/3D_Models.md)
- [Configuration and commands](wiki/Configuration_and_Commands.md)
- [Multiplayer and saved-data behavior](wiki/Multiplayer_and_Data_Safety.md)
- [Development and building](wiki/Development.md)
- [Known limitations](wiki/Known_Limitations.md)

Pages for [classes](wiki/Classes.md), [effects](wiki/Effects.md), [governments](wiki/Governments.md), [powers and abilities](wiki/Powers_and_Abilities.md), [skills](wiki/Skills.md), and [subfactions](wiki/Subfactions.md) explain that those planned systems are not implemented yet.

