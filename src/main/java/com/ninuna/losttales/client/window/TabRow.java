 ...
 * unread in a tab sits in its icon's corner ({@link TabMark}); a
 ...
 * ({@link TabSelection}); a marked tab wears the lit shape a hovered
...
public final class TabRow {
     ...
     */
    public static final int HEIGHT = PIECE_HEIGHT + 1;
    /**
     ...
     */
    public static final int LIFT = LostTalesUiSheet.TAB_SELECTED_LEFT.getHeight()
            - PIECE_HEIGHT - 1;
    /**
    ...
    public static final int ROW_HEIGHT = HEIGHT + LIFT + HEADROOM;
     ...
     */
    public static final int SELECTED_FOOT =
    ...
    private static final int INTERIOR_HEIGHT = TabIcons.SIZE;
     ...
     * text: the sheet's own, with the one shadow like every other
    ...
    /**
     * The square small controls are drawn and pressed in away from the
     * tab row — the character menu's padlock, the empty
     ...
     * layer under the border artwork's ink: the windows' inset opacity, two
    ...
    public static final class Row {
         ...
         * {@link #draggedLeft}.
         */
        public double draggedLeftExact = Double.NaN;
        /**
     ...
     * show the control.
     */
    public LostTalesUiHitBox fullscreenControlBox(Row row) {
     ...
     */
    public Hit hitAt(FontRenderer font, Row row, double mouseX, double mouseY) {
     ...
     */
    public boolean stripContains(FontRenderer font, Row row, double mouseX,
                          double mouseY) {
     ...
     */
    public void draw(FontRenderer font, PointerRegions regions, Row row,
        ...
        LostTalesUiRules.drawRuleAround(
        ...
        LostTalesUiRules.drawRule(stripLeft, stripRight,
    ...
    private float surfaceShare() {
        return this.alphaScale
                * WindowStyle.opacity(Minecraft.getMinecraft());
    }
    ...
    private int scaled(int alpha) {
        ...
        return result < LostTalesUiInk.MIN_VISIBLE_ALPHA
                ? 0 : result;
    }
    ...
    private void drawTab(FontRenderer font, Tab tab, Row row,
        ...
        try {
            ...
            boolean clipped = LostTalesUiClip.begin(
    ...
    private void drawTabIcon(final Tab tab, final int x, final float fraction,
        ...
        double iconRight = iconLeft + TabIcons.SLOT;
        final TabMark mark = TabMark.of(tab.tab);
        if (iconLeft >= contentRight) {
            return;
        }
        final int y = centredInInterior(interiorTop, TabIcons.SIZE);
        ...
        final float visible = (float)Math.max(0.0D, Math.min(1.0D,
                (contentRight - iconLeft) / TabIcons.SLOT));
        LostTalesUiFading.FadingPainter painter =
        ...
        float depth = LostTalesUiFading.sideFadeDepth(
                contentRight - clipLeft);
        LostTalesUiFading.drawFading(Minecraft.getMinecraft(),
    ...
    private void drawStripShade(Row row, List<Tab> tabs, float left,
        ...
        int alpha = scaled(LostTalesUiRules.EDGE_FADE_ALPHA);
    ...
    private void drawTabShade(Tab tab, float left, float right,
        ...
        LostTalesUiRules.drawBellFade(left, right, rowBottom - 1,
                rowTop(rowBottom),
                WindowStyle.TOP_EDGE_FADE_HEIGHT,
                scaled(Math.round(LostTalesUiRules.EDGE_FADE_ALPHA
                        * share)),
                tab.tab.tone());
    }
     ...
     * and puts the depth state back as the window screen keeps it: no test,
     * writes on.
     */
    ...
    private static void drawTabShape(float left, float right, int top,
        ...
        int surfaceRgb = LostTalesUiInk.blend(
        ...
        if (selected) {
            ...
            tipRgb = LostTalesUiInk.blend(TIP_RGB, TIP_LIT_RGB,
                    lit);
        }
        ...
        LostTalesUiInk.fillRect(left + 1, shapeTop, right - 1,
                shapeTop + 1,
                LostTalesUiInk.argb(surfaceRgb, interiorAlpha));
        ...
        if (spanRight > spanLeft) {
            ...
            LostTalesUiInk.fillRect(spanLeft,
                    ...
                    LostTalesUiInk.argb(tipRgb, spriteAlpha));
        }
        ...
        try {
            ...
            if (leftLit != null
                    && over >= LostTalesUiInk.MIN_VISIBLE_ALPHA) {
                GL11.glAlphaFunc(GL11.GL_GREATER,
            ...
            if (warm >= LostTalesUiInk.MIN_VISIBLE_ALPHA) {
                GL11.glAlphaFunc(GL11.GL_GREATER,
    ...
    private static void drawAccent(float left, float right, int y, int rgb,
                                   int alpha) {
        if (right <= left || alpha < LostTalesUiInk.MIN_VISIBLE_ALPHA) {
            return;
        }
     ...
     * else on the window screen lays its pixel art on.
     */
    public static double displayStep() {
        return 1.0D / LostTalesDisplayPixels.scaleFactor();
    }
     ...
     * bound, over the windows' animation time, fast away and gently in, as
     ...
     */
    public void holdWidthsForClose() {
     ...
     */
    public static final class ReorderLatch {
     ...
     * the pointer did.</p>
     */
    public static int reorderSlot(int left, int current, int[] restLeft,
     ...
     * a five-pixel glyph the {@link #END_CONTROL_SIZE} square small
     * controls are drawn in elsewhere.
     */
    ...
    private static int lockTop(int rowBottom) {
        return centredInStrip(rowBottom, LockAnimation.SHUT_HEIGHT)
    ...
    static LostTalesUiHitBox lockBox(int x, int rowBottom) {
        return new LostTalesUiHitBox(x, lockTop(rowBottom), LOCK_WIDTH,
                LockAnimation.HEIGHT).grown(END_CONTROL_SLACK);
    }
    ...
    private void drawLock(int x, int rowBottom, boolean locked,
        ...
        LostTalesUiButton.beginPose(motion, x, lockTop(rowBottom),
                LOCK_WIDTH, LockAnimation.HEIGHT);
        try {
    ...
    /**
     * A hairline between two of the window's controls: the windows' own
    ...
    private void drawDivider(int x, int rowBottom) {
        WindowStyle.drawDivider(x,
                centredInStrip(rowBottom, DIVIDER_HEIGHT), DIVIDER_HEIGHT,
                scaled(WindowStyle.DIVIDER_ALPHA));
    }
    ...
    static int draftLeft(int tabLeft, boolean iconShown, int labelRoom) {
        return tabLeft + PADDING_X
                + (iconShown ? TabIcons.SLOT + TabIcons.GAP
                        : 0)
     ...
     */
    static int boxWidthForNarrowestRow(Minecraft minecraft,
                                       Window window) {
        if (minecraft == null || minecraft.fontRenderer == null
                || window == null) {
            return 0;
    ...
    static boolean rowHasRoomFor(Minecraft minecraft, Window window,
        ...
        WindowPlacement.Box box = WindowPlacement.restingBounds(
