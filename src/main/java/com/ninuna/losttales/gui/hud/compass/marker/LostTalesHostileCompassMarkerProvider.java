...
public class LostTalesHostileCompassMarkerProvider implements LostTalesCompassMarkerProvider {
    ...
    public List<LostTalesCompassMarker> collectMarkers(Minecraft minecraft, float partialTicks) {
        ...
        double configuredRadius = Math.max(8.0D, LostTalesConfig.hostileCompassMarkerScanRadius);
        ...
        List<LostTalesCompassMarker> markers = new ArrayList<LostTalesCompassMarker>(trackedEnemies.size());
        ...
        for (TrackedEnemy trackedEnemy : trackedEnemies) {
            Entity entity = minecraft.theWorld.getEntityByID(trackedEnemy.getEntityId());
            if (!(entity instanceof EntityLivingBase) || entity == minecraft.thePlayer) {
                continue;
            }
            EntityLivingBase living = (EntityLivingBase) entity;
            if (!living.isEntityAlive() || living.isDead || living.dimension != minecraft.thePlayer.dimension) {
                continue;
            }
