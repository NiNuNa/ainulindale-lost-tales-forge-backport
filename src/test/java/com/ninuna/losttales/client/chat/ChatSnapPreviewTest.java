 ...
 */
public final class ChatSnapPreviewTest {
    private static final double EPSILON = 1.0E-9D;
    /** Every window stands at (600, 300) but {@code gone}, which has closed. */
    private static final ChatSnapPreview.Boxes BOXES =
            new ChatSnapPreview.Boxes() {
                @Override
                public ChatWindowPlacement.Box of(String windowId) {
                    return "gone".equals(windowId) ? null
                            : new ChatWindowPlacement.Box(600.0D, 300.0D, 160,
                                    90.0D, 37);
                }
            };
    ...
    public void thePaneStandsInTheZoneAimedAt() {
        ChatSnapPreview preview = new ChatSnapPreview();
        preview.aim("w1", ChatWindow.ScreenFill.FULL);
        ChatSnapPreview.Pane pane = preview.advance(null, window(), 960, 540);
        assertPane(2.0D, 2.0D, 958.0D, 538.0D, pane);
        assertEquals(1.0F, preview.opacity(), 0.0F);
        assertEquals("w1", preview.windowId());
        preview.aim("w1", ChatWindow.ScreenFill.RIGHT_THIRD);
    ...
    public void thePaneGoesWhenThereIsNothingToShow() {
        ChatSnapPreview preview = new ChatSnapPreview();
        preview.aim("w1", ChatWindow.ScreenFill.LEFT);
        preview.advance(null, window(), 960, 540);
        preview.aim("w1", ChatWindow.ScreenFill.NONE);
        ...
        preview.aim("w1", ChatWindow.ScreenFill.LEFT);
        preview.aim("w2", ChatWindow.ScreenFill.TOP_LEFT);
    ...
    public void aSuggestionsOtherWindowsHavePanesOfTheirOwn() {
        ChatSnapPreview preview = new ChatSnapPreview();
        Map<String, ChatWindow.ScreenFill> along =
                new LinkedHashMap<String, ChatWindow.ScreenFill>();
        along.put("w2", ChatWindow.ScreenFill.RIGHT);
        preview.aim("w1", ChatWindow.ScreenFill.LEFT, along);
        assertPane(2.0D, 2.0D, 478.0D, 538.0D,
                preview.advance(null, window(), 960, 540));
        List<ChatSnapPreview.Shown> shown = preview.advanceCompanions(null,
                BOXES, 960, 540);
        ...
        preview.aim("w1", ChatWindow.ScreenFill.TOP_LEFT);
        preview.advance(null, window(), 960, 540);
    ...
    public void aClosedWindowsPaneGoesWithIt() {
        ...
        Map<String, ChatWindow.ScreenFill> along =
                new LinkedHashMap<String, ChatWindow.ScreenFill>();
        along.put("w2", ChatWindow.ScreenFill.TOP_RIGHT);
        along.put("gone", ChatWindow.ScreenFill.BOTTOM_RIGHT);
        preview.aim("w1", ChatWindow.ScreenFill.LEFT, along);
        preview.advance(null, window(), 960, 540);
        List<ChatSnapPreview.Shown> shown = preview.advanceCompanions(null,
                BOXES, 960, 540);
    ...
    public void everyEdgeTravelsTheSameShareOfItsWay() {
        ChatSnapPreview.Pane from = new ChatSnapPreview.Pane(100.0D, 200.0D,
                260.0D, 290.0D);
        ChatSnapPreview.Pane to = new ChatSnapPreview.Pane(2.0D, 2.0D, 478.0D,
                538.0D);
    ...
    private static ChatWindowPlacement.Box window() {
        return new ChatWindowPlacement.Box(100.0D, 200.0D, 160, 90.0D, 37);
    }

    private static void assertPane(double left, double top, double right,
                                   double bottom, ChatSnapPreview.Pane pane) {
