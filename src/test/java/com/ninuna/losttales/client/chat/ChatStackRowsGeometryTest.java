 ...
 * between two runs is a third of one, the divider's row is a line, and
...
public final class ChatStackRowsGeometryTest {
    ...
    private static final int THIRD = ChatStackRows.SPACER_HEIGHT;

    @Test
    public void aBlankRowIsAThirdOfALine() {
        assertEquals(4, THIRD);
        assertEquals(11, LINE);
        assertEquals(THIRD, ChatStackRows.heightOf(
    ...
    public void rowsStandOnTheRowsBelowThem() {
        ...
        assertEquals(LINE + THIRD, rows.top(2));
        assertEquals(2 * LINE + THIRD, rows.top(3));
        assertEquals(3 * LINE + THIRD, rows.top(4));
        assertEquals(3 * LINE + 2 * THIRD, rows.top(5));
        assertEquals(4 * LINE + 2 * THIRD, rows.total());
        assertEquals(rows.total(), rows.top(6));
        assertEquals(rows.total(), rows.top(99));
        assertEquals(THIRD, rows.height(1));
    ...
    public void theDividerRowIsAWholeLine() {
        ...
        assertEquals(3 * LINE + THIRD, rows.top(4));
        assertEquals(4 * LINE + THIRD, rows.total());
    }

    @Test
    public void scrollOffsetsMapOntoDistancesAndBack() {
        ChatStackRows rows = new ChatStackRows();
        rows.reset(new int[] {LINE, THIRD, LINE, LINE});
        ...
        assertEquals(LINE + THIRD / 2.0D, rows.offsetOf(1.5D), 1.0E-9D);
        assertEquals(LINE + THIRD, rows.offsetOf(2.0D), 1.0E-9D);
        assertEquals(3 * LINE + THIRD, rows.offsetOf(4.0D), 1.0E-9D);
        // Past the stack, rows are whole lines of nothing.
        assertEquals(4 * LINE + THIRD, rows.offsetOf(5.0D), 1.0E-9D);
    ...
    public void theHighestRowBelowALimit() {
        ChatStackRows rows = new ChatStackRows();
        rows.reset(new int[] {LINE, THIRD, LINE});
        ...
        assertEquals(2, rows.lastRowBelow(LINE + THIRD + 1.0D));
