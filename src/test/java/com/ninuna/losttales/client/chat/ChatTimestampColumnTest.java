...
/**
 * The timestamp area holds the avatar with at least five clear pixels
...
public final class ChatTimestampColumnTest {
    ...
    public void theAvatarStandsFivePixelsFromEitherSide() {
        ...
        assertEquals(5.0F, column.avatarX(), 0.0F);
        // From the sphere's last pixel to the separator.
        assertEquals(5.0F, column.separatorX()
    ...
    /**
     * The widest time as drawn, with a clear display pixel either side:
     * the avatar's own area holds it at GUI scale 2, and the area widens
     * to thirty at 3, thirty-four at 4, thirty-six at 5 and forty-six at
     * 1, where there is no smaller text.
     */
    @Test
    public void theAreaHoldsTheWidestTimeAtEveryScale() {
        assertEquals(28, width(0.5F, 2.0F));
        assertEquals(30, width(2.0F / 3.0F, 3.0F));
        assertEquals(34, width(0.75F, 4.0F));
        assertEquals(36, width(0.8F, 5.0F));
        assertEquals(46, width(1.0F, 1.0F));
    }
    ...
    public void aWiderAreaCentresTheAvatar() {
        ...
        assertEquals(6.0F, scaleThree.avatarX(), 0.0F);
        assertEquals(6.0F, scaleThree.separatorX()
        ...
        assertEquals(8.0F, scaleFour.avatarX(), 0.0F);
        assertEquals(8.0F, scaleFour.separatorX()
    ...
    public void aTimeIsCentredByItsInk() {
        ChatTimestampColumn column = ChatTimestampColumn.forTimeWidth(0.0F);
        assertEquals(5.0F, column.timeX(18.0F), 0.0F);
    }
