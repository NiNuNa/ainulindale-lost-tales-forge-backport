...
/**
 * A window's tool strip: the area's chevron under the tab search, the
...
public final class ChatToolStripTest {
    ...
    public void theWellStandsShortAtTheStripsRightBeforeTheListsButton() {
        ...
        assertEquals(400 - 3 - ChatToolStrip.MEMBERS_GLYPH_WIDTH, laid.membersX);
        assertEquals(laid.membersX - 5, laid.wellRight);
        assertEquals(ChatToolStrip.WELL_WIDTH, laid.wellRight - laid.wellLeft);
        // The chevron stands centred under the tab search, the odd pixel left.
        int chevron = LostTalesUiSheet.TOGGLE_1.getWidth();
        assertEquals(3 + Math.floorDiv(15 - chevron, 2), laid.areaX);
