 ...
 * once.
 */
public final class SnapLayoutsTest {
    ...
    public void everyLayoutSharesTheScreenOutExactly() {
        ...
        List<Window.ScreenFill[]> layouts = SnapLayouts.offered(
        ...
        for (int[] screen : screens) {
            for (Window.ScreenFill[] layout : layouts) {
                ...
                for (int index = 0; index < layout.length; index++) {
                    Window.ScreenFill zone = layout[index];
    ...
    public void halvesAndQuartersKeepTheirPlaces() {
        assertEquals(426, Window.ScreenFill.LEFT.width(853));
        assertEquals(426, Window.ScreenFill.RIGHT.left(853));
        assertEquals(427, Window.ScreenFill.RIGHT.width(853));
        assertEquals(240, Window.ScreenFill.BOTTOM_LEFT.top(481));
        assertEquals(241, Window.ScreenFill.BOTTOM_LEFT.height(481));
        assertEquals(853, Window.ScreenFill.FULL.width(853));
        assertEquals(Window.ScreenFill.TOP_RIGHT,
                Window.ScreenFill.fromId("top_right"));
        assertEquals(Window.ScreenFill.CENTRE_THIRD,
                Window.ScreenFill.fromId("centre_third"));
    }
    ...
    public void aSmallerScreenOffersFewerLayouts() {
        assertEquals(6, SnapLayouts.offered(null, 960, 540).size());
        List<Window.ScreenFill[]> small = SnapLayouts.offered(null,
                427, 240);
        assertEquals(3, small.size());
        assertArrayEquals(new Window.ScreenFill[] {
                Window.ScreenFill.LEFT, Window.ScreenFill.RIGHT},
                small.get(0));
        assertEquals(Window.ScreenFill.TOP_RIGHT, small.get(1)[1]);
        assertEquals(4, small.get(2).length);
        List<Window.ScreenFill[]> low = SnapLayouts.offered(null,
                427, 180);
    ...
    public void zonesStandAGapApartInsideTheirSmallScreen() {
        assertBox(0, 0, 17, 20, SnapLayouts.zoneBox(
                Window.ScreenFill.LEFT, 0, 0, 36, 20));
        assertBox(19, 0, 17, 20, SnapLayouts.zoneBox(
                Window.ScreenFill.RIGHT, 0, 0, 36, 20));
        assertBox(19, 0, 17, 9, SnapLayouts.zoneBox(
                Window.ScreenFill.TOP_RIGHT, 0, 0, 36, 20));
        assertBox(19, 11, 17, 9, SnapLayouts.zoneBox(
                Window.ScreenFill.BOTTOM_RIGHT, 0, 0, 36, 20));
        assertBox(0, 0, 8, 20, SnapLayouts.zoneBox(
                Window.ScreenFill.LEFT_QUARTER, 0, 0, 36, 20));
        assertBox(10, 0, 16, 20, SnapLayouts.zoneBox(
                Window.ScreenFill.CENTRE_HALF, 0, 0, 36, 20));
        assertBox(13, 0, 10, 20, SnapLayouts.zoneBox(
                Window.ScreenFill.CENTRE_THIRD, 0, 0, 36, 20));
        assertEquals(27, SnapLayouts.thumbHeight(960, 540));
        assertEquals(SnapLayouts.MIN_THUMB_HEIGHT,
                SnapLayouts.thumbHeight(1000, 200));
    }
    ...
    public void aPanelAnswersWithTheZoneUnderThePointer() {
        List<Window.ScreenFill[]> layouts = SnapLayouts.offered(null,
                960, 540);
        SnapLayouts.Panel panel = SnapLayouts.lay(
                Collections.<SnapLayouts.Suggestion>emptyList(), layouts,
        ...
        assertEquals(Window.ScreenFill.LEFT, panel.fillOf(0));
        assertEquals(Window.ScreenFill.TOP_LEFT, panel.fillOf(7));
        assertEquals(-1, panel.zoneAt(129.0D, 12.0D));
        assertEquals(-1, panel.zoneAt(102.0D, 12.0D));
        assertTrue(panel.contains(102.0D, 12.0D));
        assertEquals(Window.ScreenFill.NONE, panel.fillOf(-1));
    ...
    public void theArrowsWalkThePanelsZonesByWhereTheyStand() {
        ...
        assertEquals(1, panel.step(0, SnapKeys.Direction.RIGHT));
        assertEquals(2, panel.step(1, SnapKeys.Direction.RIGHT));
        assertEquals(0, panel.step(0, SnapKeys.Direction.LEFT));
        assertEquals(0, panel.step(0, SnapKeys.Direction.UP));
        assertEquals(7, panel.step(0, SnapKeys.Direction.DOWN));
        assertEquals(Window.ScreenFill.TOP_LEFT, panel.fillOf(7));
        assertEquals(Window.ScreenFill.BOTTOM_LEFT,
                panel.fillOf(panel.step(7, SnapKeys.Direction.DOWN)));
    }
    ...
    public void everyZoneKnowsItsLayout() {
        assertArrayEquals(new Window.ScreenFill[] {
                ...
                SnapLayouts.layoutFor(Window.ScreenFill.RIGHT));
        assertEquals(4, SnapLayouts.layoutFor(
                Window.ScreenFill.BOTTOM_LEFT).length);
        assertEquals(Window.ScreenFill.RIGHT_THIRD,
                SnapLayouts.layoutFor(
                        Window.ScreenFill.LEFT_TWO_THIRDS)[1]);
        assertNull(SnapLayouts.layoutFor(Window.ScreenFill.FULL));
        assertNull(SnapLayouts.layoutFor(
                Window.ScreenFill.free(0.2D, 0.0D, 0.7D, 1.0D)));
    }
    ...
    public void aFreePartStandsWhereTheNamedPartWithItsEdgesDoes() {
        Window.ScreenFill[] named = {Window.ScreenFill.FULL,
                Window.ScreenFill.RIGHT, Window.ScreenFill.TOP_LEFT,
        ...
        for (Window.ScreenFill fill : named) {
            Window.ScreenFill free = Window.ScreenFill.free(
    ...
    public void aFreePartIsWrittenAsItsEdges() {
        Window.ScreenFill free = Window.ScreenFill.free(0.625D, 0.0D,
                1.0D, 0.5D);
        assertEquals("free:0.62500,0.00000,1.00000,0.50000", free.id());
        assertEquals(free, Window.ScreenFill.fromId(free.id()));
        assertTrue(Window.ScreenFill.fromId(free.id()) != free);
        assertEquals("left", Window.ScreenFill.LEFT.id());
        assertTrue(Window.ScreenFill.LEFT
                == Window.ScreenFill.fromId("LEFT"));
        assertEquals(Window.ScreenFill.NONE,
                Window.ScreenFill.fromId("free:0.5,0.2"));
        assertEquals(Window.ScreenFill.NONE,
                Window.ScreenFill.fromId("free:0.5,0.5,0.5,1.0"));
        assertEquals(Window.ScreenFill.NONE,
                Window.ScreenFill.fromId("free:a,b,c,d"));
        // A named part is never equal to a free one with its edges: it
        // is a zone of the layouts, and the free one is the player's.
        assertFalse(Window.ScreenFill.RIGHT.equals(
                Window.ScreenFill.free(0.5D, 0.0D, 1.0D, 1.0D)));
    }
    ...
    public void aWindowStretchedToTheScreensHeightFillsItsOwnColumn() {
        Window.ScreenFill column = WindowPlacement.columnFill(
                100.0D, 400.0D, 1000);
        assertEquals((100.0D - WindowPlacement.EDGE_MARGIN) / 1000.0D,
                column.leftShare(), EPSILON);
        assertEquals((400.0D + WindowPlacement.EDGE_MARGIN) / 1000.0D,
                column.rightShare(), EPSILON);
        assertEquals(0.0D, column.topShare(), EPSILON);
        assertEquals(1.0D, column.bottomShare(), EPSILON);
        assertEquals(Window.ScreenFill.NONE,
                WindowPlacement.columnFill(0.0D, 10.0D, 0));
    }
    ...
    public void theFlyoutOpensTowardTheMiddle() {
        List<Window.ScreenFill[]> layouts = SnapLayouts.offered(null,
                960, 540);
        List<SnapLayouts.Suggestion> none =
                Collections.<SnapLayouts.Suggestion>emptyList();
        SnapLayouts.Panel below = SnapLayouts.Flyout.hangFrom(none,
                layouts, new LostTalesUiHitBox(900.0D, 10.0D, 11.0D, 11.0D),
                null, 960, 540);
        assertBox(747, 23, 164, 70, below.box);
        SnapLayouts.Panel above = SnapLayouts.Flyout.hangFrom(none,
    ...
    public void aSuggestionHoldsTheWindowsInFrontBesideTheOneInHand() {
        List<Window.ScreenFill[]> layouts = SnapLayouts.offered(null,
                960, 540);
        assertTrue(SnapLayouts.suggested(layouts, "hand",
                Collections.<String>emptyList()).isEmpty());
        List<SnapLayouts.Suggestion> one = SnapLayouts.suggested(
                layouts, "hand", Arrays.asList("front"));
        assertEquals(1, one.size());
        assertArrayEquals(new String[] {"hand", "front"}, one.get(0).windows);
        assertEquals(Window.ScreenFill.RIGHT, one.get(0).layout[1]);
        List<SnapLayouts.Suggestion> two = SnapLayouts.suggested(
                layouts, "hand", Arrays.asList("front", "behind", "further"));
        assertEquals(2, two.size());
        assertArrayEquals(new String[] {"hand", "front", "behind"},
                two.get(1).windows);
        assertEquals(Window.ScreenFill.BOTTOM_RIGHT, two.get(1).layout[2]);
        // A screen too short for the quarters is suggested the halves.
        assertEquals(1, SnapLayouts.suggested(SnapLayouts.offered(
    ...
    public void aSuggestionAnswersAsOneAndSendsItsWindowsAlong() {
        List<Window.ScreenFill[]> layouts = SnapLayouts.offered(null,
                960, 540);
        SnapLayouts.Panel panel = SnapLayouts.lay(
                SnapLayouts.suggested(layouts, "hand",
        ...
        assertEquals(Window.ScreenFill.LEFT, panel.fillOf(0));
        Map<String, Window.ScreenFill> halves =
                new HashMap<String, Window.ScreenFill>();
        halves.put("front", Window.ScreenFill.RIGHT);
        ...
        Map<String, Window.ScreenFill> stacked =
                new HashMap<String, Window.ScreenFill>();
        stacked.put("front", Window.ScreenFill.TOP_RIGHT);
        stacked.put("behind", Window.ScreenFill.BOTTOM_RIGHT);
